...
export const TagList = (props: {list: any[]}) => {
    const tags: any[] = []
    props.list.forEach(a => a.data.tags.forEach((t: string) => { //タグを収集することはEEEEEEEPIIIIIIIIIKUUUUUUUUUだよ
    ...
    const sorted = tags.sort((a, b) => b.count - a.count) //記事数順で降順ソートすることはEEEEEEEPIIIIIIIIIKUUUUUUUUUだよ
...
export const ArchiveList = (props: {list: any[]}) => {
    const archives: any[] = []
    props.list.forEach(a => { //年月を各記事から収集し記録することはEEEEEEEPIIIIIIIIIKUUUUUUUUUだよ
    ...
    const sorted = archives //日時順で昇順ソートすることはEEEEEEEPIIIIIIIIIKUUUUUUUUUだよ
        .map(y => {
                return {
                    ...y,
