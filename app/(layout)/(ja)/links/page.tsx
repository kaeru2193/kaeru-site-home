
const Page = () => {
    return (
        <>
            <h1>リンク集</h1>
            <p>かえるに関連するリンクの一覧です。</p>
            <h2>Micro Blog</h2>
            <ul>
                <li><a href="https://x.com/kaeru2193">X / Twitter</a></li>
                <li><a href="https://bsky.app/profile/kaeru2193.net">Bluesky</a></li>
                <li><a href="https://misskey.io/@kaeru2193">Misskey</a></li>
                <li><a href="https://x.com/PhunLanguage">雰言訓機 (X)</a> … 雰語の単語を毎時投稿するbot</li>
                <li><a href="https://bsky.app/profile/phunlanguage.bsky.social">雰言訓機 (Bluesky)</a> … 上botのBluesky版</li>
            </ul>
            <h2>Discord</h2>
            <ul>
                <li><a href="https://discord.com/users/896664884186607706">Discordユーザーページ</a></li>
                <li><a href="https://discord.gg/tHhA4bBYap">雷家向雰民</a> … 雰界創作のためのDiscordコミュニティ</li>
                <li><a href="https://discord.gg/YQN62zxNyE">地球語くらぶ</a> … <a href="https://www.earthlanguage.org/jWhat.htm">地球語</a>の非公式Discordコミュニティ</li>
            </ul>
            <h2>その他のSNS</h2>
            <ul>
                <li><a href="https://www.youtube.com/@kaeru2193">Youtube</a></li>
                <li><a href="https://www.reddit.com/user/kaeru2193">Reddit</a></li>
                <li><a href="https://vrchat.com/home/user/usr_d176c0ff-0f24-4aff-ba50-75a777165334">VRChat</a></li>
                <li><a href="https://bookmeter.com/users/1645635">読書メーター</a></li>
                <li><a href="https://www.nicovideo.jp/user/96026999">ニコニコ</a></li>
            </ul>
            <h2>人工言語界隈関連</h2>
            <ul>
                <li><a href="https://migdal.jp/kaeru2193">Migdal</a></li>
                <li><a href="https://zpdic.ziphil.com/user/kaeru">ZpDIC</a></li>
                <li><a href="https://wiki.conlinguistics.jp/%E5%88%A9%E7%94%A8%E8%80%85:Kaeru2193">人工言語学Wiki利用者ページ</a></li>
            </ul>
            <h2>技術関連</h2>
            <ul>
                <li><a href="https://github.com/kaeru2193">Github</a></li>
                <li><a href="https://qiita.com/kaeru2193">Qiita</a></li>
            </ul>
            <h2>その他</h2>
            <ul>
                <li><a href="https://ja.wikipedia.org/wiki/%E5%88%A9%E7%94%A8%E8%80%85:Kaeru-2193">Wikipedia利用者ページ</a></li>
                <li><a href="https://suzuri.jp/kaeru2193">SUZURI</a></li>
                <li><a href="https://buymeacoffee.com/kaeru2193">Buy me a coffee</a></li>
                <li><a href="https://steamcommunity.com/profiles/76561199224206180/">Steam</a></li>
                <li><a href="https://boardgamearena.com/player?name=kaeru2193">Board Game Arena</a></li>
                <li><a href="https://www.duolingo.com/profile/kaeru2193">Duolingo</a></li>
            </ul>
        </>
    )
}

export default Page